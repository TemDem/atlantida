...
const buttons = document.querySelectorAll(".slide-content__button");
const arrodDowwButtons = document.querySelectorAll(".slide-content__down");
const header = document.querySelector(".header");
const atlantidaSlider = document.querySelector(".atlantida-slider");
const about = document.querySelector('.about')
if (buttons && header && atlantidaSlider && arrodDowwButtons && about) {
  buttons.forEach(el => el.addEventListener("click", scrollSlider));
  arrodDowwButtons.forEach(el => el.addEventListener("click", scrollSlider));
}
function scrollSlider() {
  // const paddingTop = parseFloat(window.getComputedStyle(about).paddingTop);
  const headerHeight = header.offsetHeight;
  const atlantidaSliderHeight = atlantidaSlider.offsetHeight;
  window.scrollTo({
    top: atlantidaSliderHeight - headerHeight, // + paddingTop,
    left: 0,
    behavior: "smooth",
  });
}
const map = document.getElementById('map')
let center = map.dataset.coordinates.split(',');
if (center) {
	function init() {
...
ymaps.ready(init);
// function tabs(buttons) {
// 	if (buttons.length > 0) {
// 		buttons.forEach(button => button.addEventListener('click', e => {
// 			const click = e.target
// 			const activeTab = document.querySelector('._tab-active');
// 			const tabActive = document.querySelector(click.dataset.tabs)
// 			activeTab.classList.remove("_tab-active");
// 			tabActive.classList.add("_tab-active");
// 	  }));
//   }
// }
//  tabs(document.querySelectorAll("[data-tabs]"));

const elem = document.querySelector(".tabs-images");
const iso = new Isotope(elem, {
  // options
  itemSelector: ".tabs-images__item",
  layoutMode: "masonry",
});
document.querySelectorAll(".portfolio-tabs__tab").forEach(el => {
...
const atlantidaSliderSwiper = new Swiper(".atlantida-slider__slider", {
  navigation: {
    nextEl: ".slider__next",
    prevEl: ".slider__prev",
  },
...
const reviewsSliderSwiper = new Swiper(".slider-reviews", {
  pagination: {
